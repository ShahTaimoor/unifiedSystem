...
const adaptProduct = (product) => {
  ...
  if (adapted.category) {
    adapted.category = {
      ...
      active: adapted.category.active !== undefined ? adapted.category.active : adapted.category.isActive,
    };
  }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const data = Array.isArray(result.products) ? result.products.map(adaptProduct) : [];
      const pagination = {
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const categoriesResult = await categoryService.getCategories({ search: req.query.search, page: 1, limit: 9999, isActive: true });
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await salesOrderRepository.findWithPagination({}, { page, limit });
      res.json({
  ...
  async (req, res, next) => {
    try {
      ...
      if (['confirmed', 'partially_invoiced', 'fully_invoiced'].includes(salesOrder.status)) {
        return res.status(400).json({ message: 'Cannot update order after confirmation or invoicing' });
      }
      const updateData = {
        status,
      ...
      if (packerName) {
        updateData.notes = `${salesOrder.notes || ''} ${packerName ? `Packer: ${packerName}` : ''}`.trim();
      }
      const updatedOrder = await salesOrderRepository.update(req.params.id, updateData);
...
router.delete(
  ...
  auth,
  requirePermission('delete_sales_orders'),
  [body('orderIds').isArray({ min: 1 }).withMessage('orderIds array is required')],
  handleValidation,
  async (req, res, next) => {
    try {
      ...
      for (const id of orderIds) {
        ...
        if (salesOrder.status !== 'draft') {
          results.push({ id, success: false, error: 'Only draft orders can be deleted' });
          continue;
        }
...
router.post(
  ...
  [
    ...
    body('products.*.id').isUUID(4).withMessage('Each product must include a valid ID'),
    body('products.*.quantity').isInt({ min: 1 }).withMessage('Each product must include a positive quantity'),
