  ...
  const handleExport = async () => {
    try {
      ...
      const doc = new jsPDF({
        ...
        unit: "mm",
        format: payload.paperSize || "a4",
      });
      let currentY = 10;
      const startX = 5;
      const pageWidth = doc.internal.pageSize.getWidth();
      const pageHeight = doc.internal.pageSize.getHeight();
      const smallFont = payload.columns?.length > 14 ? 7 : 8;
      ...
      autoTable(doc, {
        ...
        didDrawCell: function (data) {
          if (
            ...
            imageColumnsIndices.includes(data.column.index)
          ) {
      ...
      // Footer / Signatures
      let finalY =
        (doc.lastAutoTable ? doc.lastAutoTable.finalY : currentY) + 25;
      ...
      let filename =
        payload.filename || `Export_${new Date().toLocaleDateString()}.pdf`;
