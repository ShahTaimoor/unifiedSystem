/**
 * Resolve product/category image URLs for the storefront.
 *
 * The API often returns absolute URLs like http://localhost:5000/api/images/...
 * while the SPA runs on another port (e.g. 5174). Using that URL as <img src>
 * is cross-origin; mixed content (https page + http image) also breaks loads.
 * Paths under /api are rewritten to the current page origin so the Vite proxy
 * can serve them (dev) or same-origin routing can apply (prod behind one host).
 */
