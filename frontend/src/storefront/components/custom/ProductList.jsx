...
import {
  fetchProducts,
  searchProducts,
} from "@/storefront/redux/slices/products/productSlice";
import {
  Link,
  useNavigate,
  useLocation,
...
import {
  ...
  getSpacerHeightClassName,
} from "@/storefront/utils/classNameHelpers";
...
const CartProduct = ({ product, quantity }) => {
  ...
  const availableStock =
    product.availableStock !== undefined ? product.availableStock : stock;
...
const ProductList = () => {
      ...
      setQuantities((prev) => {
        ...
        cartItems.forEach((item) => {
          ...
          if (productId && item.quantity) {
            ...
            if (
              updatedQuantities[productId] === undefined ||
  ...
  const handleAddToCart = useCallback(
    (product) => {
      ...
      if (qty <= 0 || qty > product.stock) {
        toast.error(
          `Please select a valid quantity. Available stock: ${product.stock}`,
        );
        return;
      }
      ...
      dispatch(
        ...
        .catch((error) => {
          const errorMessage =
            error?.message || error || "Failed to add item to cart";
          ...
          // Fallback to local cart when backend cart API is unavailable or returns 404
          if (
            error?.response?.status === 404 ||
            errorMessage.toString().toLowerCase().includes("not found") ||
            errorMessage.toString().includes("404") ||
            errorMessage.toString().toLowerCase().includes("network error")
          ) {
            dispatch(addItemLocal({ product, quantity: qty }));
            toast.success(
              `${qty} ${qty === 1 ? "item" : "items"} added to cart!`,
            );
            return;
  ...
  const loadingProducts = isSearchMode
            ...
            <h2 className="text-lg font-semibold text-gray-900">
              ...
              {displayPagination.total > 0 && (
                <span className="text-sm font-normal text-gray-500 ml-2">
                  ...
                  {displayPagination.total === 1 ? "product" : "products"})
                </span>
              )}
            </h2>
            <button
