...
const Footer = () => {
  ...
  const dynamicSocialMedia = useMemo(() => {
    const list = [];
    
    // Facebook
    if (company.showFacebook !== false) {
      const fbHref = company.facebookLink || (SOCIAL_MEDIA[0] && SOCIAL_MEDIA[0].href !== '#' ? SOCIAL_MEDIA[0].href : '');
      if (fbHref) {
        list.push({
          name: 'Facebook',
          icon: 'facebook',
    ...
    // Instagram
    if (company.showInstagram !== false) {
      const igHref = company.instagramLink || (SOCIAL_MEDIA[1] && SOCIAL_MEDIA[1].href !== '#' ? SOCIAL_MEDIA[1].href : '');
      if (igHref) {
        list.push({
          name: 'Instagram',
          icon: 'instagram',
    ...
    // WhatsApp
    if (company.showWhatsapp !== false) {
      const waNumber = company.whatsappNumber || (CONTACT_INFO.whatsapp?.[0] || '');
      if (waNumber) {
        ...
        list.push({
          ...
          href: `https://wa.me/${cleanWa.startsWith('+') ? cleanWa.slice(1) : cleanWa}`,
        });
      }
    }
    
    // TikTok
    if (company.showTiktok !== false && company.tiktokLink) {
      list.push({
        name: 'TikTok',
        icon: 'tiktok',
    ...
    return list;
  }, [company, SOCIAL_MEDIA, CONTACT_INFO]);
        ...
        <div className="container mx-auto px-4 pt-2 pb-8 lg:pt-8 lg:pb-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              {/* Social Media Links */}
              {dynamicSocialMedia.length > 0 && (
                <div className="flex space-x-4">
                  {dynamicSocialMedia.map((social) => (
                    <a
                      key={social.name}
                      href={social.href}
                      target="_blank"
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="space-y-1">
                        <p className="text-slate-300 text-sm">
                          {company.whatsappNumber || CONTACT_INFO.whatsapp[0] || company.phone}
                        </p>
                      </div>
                    </div>
                  )}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {/* Interactive Map */}
                {company.showMapLocation !== false && (company.mapLocation || LOCATION.mapEmbedUrl) && (
                  <div className="mt-4 overflow-hidden rounded-lg border border-slate-700/50">
                    <iframe
                      src={company.mapLocation || LOCATION.mapEmbedUrl}
